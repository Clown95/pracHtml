...
function commonAjaxSubmit(url, formObj, plus, fn) {
    ...
    $(formObj).ajaxSubmit({
        url: url, data: plus, type: "POST", success: function (data, st) {
            if (data.status == 1) {
...
$(function () {
    $("#navleft").hover(function () {
        $(this).addClass("navlefthover");
    }, function () {
        $(this).removeClass("navlefthover");
    })
    $(".ltitimg").hover(function () {
        $(this).addClass("hover");
    }, function () {
...
;
var
    windowWidth, windowHeight, popstatus, outTime;
function whWindow() {
...
function popStatuRe() {
    $('body #wstatus').css({
        'left': (windowWidth / 2) - ($('#wstatus').width() + 18) / 2 + 'px',
        'top': (windowHeight / 2) - ($('#wstatus').height() + 18) / 2 + 'px'
    });
};
/*!
 * jQuery Form Plugin
...
window.popup = window.asyncbox = {
    ...
    minWidth: 320,
    maxWidth: 700,
    Cover: {opacity: 0.7, background: '#000'},
    Language: {OK: '确定(O)', NO: '否(N)', YES: '是(Y)', CANCEL: '取消(C)', CLOSE: '关闭(C)'}
};
(function (a) {
    function c(a) {
    ...
    function h(a) {
        if (a.args) {
            ...
            return d.href = a.url, c = d.href, b = typeof a.args == "string" ? a.args : g(a.args), c.indexOf("#") >= 0 && (c = c.substr(0, c.indexOf("#"))), c.indexOf("?") >= 0 && (c = c.substr(0, c.indexOf("?"))), c + d.search + (d.search ? "&" + b : "?" + b) + d.hash
        }
        return a.url
    }
        ...
        a(function () {
            if ($) {
                ...
                if (e) {
                    ...
                    a(Q).append("<div id=" + d + ' class="asyncbox" style="top:-5000px;left:-5000px;z-index:' + D.zIndex++ + '">' + n(b) + "</div>"), b.abo = a("#" + d)[0];
                    var h = 0, i, j = b.abo.getElementsByTagName("*"), l = j.length;
                    for (; h < l; h++)i = j[h].className, i && (i = i.split("ab_border ab_")[1] || i.split("ab_")[1], b.node[i] = j[h]);
                    if (Y && b.node.lt.currentStyle.png) {
                        var e, o, p, q = b.node.outer;
                        for (var r = 0; r < 3; r++)for (var u = 0; u < 3; u++)e = q.rows[r].cells[u], p = e.currentStyle.png, p && (o = e.runtimeStyle, o.backgroundImage = "none", o.filter = "progid:DXImageTransform.Microsoft.AlphaImageLoader(src='" + $ + "skins/" + p + "',sizingMethod='scale')")
                    }
    ...
    function k(b) {
        function f(a) {
            y && (y = !1, x.display = "block"), u = a.clientX - s, v = a.clientY - t, u < o ? u = o : u > r && (u = r), v < n ? v = n : v > p && (v = p), v < n && (v = n), u < o && (u = o), els.top = v + "px", els.left = u + "px"
        }
        ...
        function g() {
            l(!1), b.drag.clone && (D.Flash ? q(i, {
                ...
                left: el.offsetLeft,
                fixed: b.fixed
            }) : (j.top = el.offsetTop + "px", j.left = el.offsetLeft + "px"), x.display = "none"), Y && b.fixed && e(i), X && el.releaseCapture ? (el.releaseCapture(), el.onmousemove = null, el.onmouseup = null) : a(F).unbind("mousemove", f).unbind("mouseup", g)
        }

        var h = b.id, i = el = c(h), j = els = el.style, k, m, n, o, p, r, s, t, u, v, n, o, p, r, w = T, x = w.style, y = !1;
        a("#" + h + "_header").css({cursor: "move"}), a("#" + h + "_header").mousedown(function (c) {
    ...
    function o(a) {
        var b = a.content;
        return typeof b == "object" && b ? (a.wrap = {key: b, value: b.innerHTML}, b.innerHTML = "", a.wrap.value) : b
    }
    ...
    function w(b) {
        if (b.pageMode) {
            ...
            e.options = b, e.data = b.data, e.opener = b.opener || window, e.close = function () {
                ...
                return D.opener(a)
            }, e.buttons = b.buttons ? a("#" + b.id + "_btnsbar").find("a") : "undefined", d.api = e, c.one("load", function () {
                try {
    ...
    function y(a, b) {
        ...
        a.center ? i = (p - l) / 2 : i = l > p / 2 ? (p - l) / 2 : p * .382 - l / 2, !Y && a.fixed ? (g = i, h = (o - k) / 2, a.top >= 0 && (g = a.top), a.left >= 0 && (h = a.left), a.right >= 0 && (h = o - k - a.right), a.bottom >= 0 && (g = p - l - a.bottom), g = g < 0 ? 0 : g, h = h < 0 ? 0 : h) : (g = m + i, h = n + (o - k) / 2, a.top >= 0 && (g = m + a.top), a.left >= 0 && (h = n + a.left), a.right >= 0 && (h = n + o - k - a.right), a.bottom >= 0 && (g = m + p - l - a.bottom), g = g < m ? m : g, h = h < n ? n : h), b ? q(c, {
            top: g,
            left: h
    ...
    function A(b) {
        var c = new Object;
        c.id = b.id, c.top = b.top, c.right = b.right, c.bottom = b.bottom, c.left = b.left, c.flash = b.flash, M.push(c), M.length > 0 && !H && (a(E).bind("resize", bb), H = !0)
    }
    ...
    function C(a, b, c, d) {
        var e = G + d, f = {id: e, icon: e, title: b, reset: !0, modal: !0, content: a, callback: c};
    ...
    if (document.compatMode == "CSS1Compat" && $) {
        ...
        Q.setAttribute("version", "1.5"), Q.setAttribute("url", "http://blog.51edm.org"), R.innerHTML = "<ul><li><a><b></b></a></li></ul>", R.style.cssText = "position:absolute;top:-5000px;left:-5000px", S.style.cssText = cb, S.setAttribute("unselectable", "on"), S.onmousedown = function () {
            return !1
        }, T.style.cssText = "position:absolute;z-index:" + (D.zIndex + 900) + ";display:none", U.style.cssText = cb + "cursor:move;background:#fff;opacity:0;filter:alpha(opacity=0);z-index:" + (D.zIndex + 1e3), a(function () {
    ...
    var bb = function () {
        if (V != N.clientWidth || W != N.clientHeight)V = N.clientWidth, W = N.clientHeight, a.each(M, function (a) {
            var b = {}, d = M[a];
            b.abo = c(d.id), b.id = d.id, b.top = d.top, b.left = d.left, b.right = d.right, b.bottom = d.bottom, D.Flash && d.flash ? y(b, !0) : y(b, !1)
        })
    };
    D.btn = {
        OK: [{value: D.Language.OK, result: "ok"}],
        NO: [{value: D.Language.NO, result: "no"}],
        YES: [{value: D.Language.YES, result: "yes"}],
        CLOSE: [{title: D.Language.CLOSE, result: "close"}],
        CANCEL: [{value: D.Language.CANCEL, result: "cancel"}]
    }, D.btn.OKCANCEL = D.btn.OK.concat(D.btn.CANCEL), D.btn.YESNO = D.btn.YES.concat(D.btn.NO), D.btn.YESNOCANCEL = D.btn.YES.concat(D.btn.NO).concat(D.btn.CANCEL), D.cover = function (b, c) {
        var d = a(S), e = S.style;
        b ? (I = b, e.zIndex = c || D.zIndex, D.Flash ? d.fadeTo(500, D.Cover.opacity) : d.show()) : (I = b, D.Flash ? d.fadeOut(300) : d.hide(), J = [])
    }, D.close = function (d) {
        var e = c(d);
        e && (M.length > 0 && (M = B(M, d, "id")), H && M.length == 0 && (a(E).unbind("resize", bb), H = !1, M = []), a.each(L, function (f, g) {
            if (g.id == d) {
                if (I)for (b in K)K[b] == d && (K = B(K, d), J.length > 1 && K.length != 0 ? (J.pop(), D.cover(!0, J[J.length - 1])) : D.cover(!1));
                g.wrap && (c(g.id + "_content").innerHTML = "", p(g)), g.timeobj = null, g.cache ? a(e).hide() : (L.length > 0 && (L = B(L, d, "id")), g.pageMode && u(d), a(e).remove()), g.unload()
            }
        }))
